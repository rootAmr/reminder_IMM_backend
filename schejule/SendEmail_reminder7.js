...
export const sendReminder7 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder7 !== 'berhasil7') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder7: 'berhasil7' });
                                console.log('Status updated to "berhasil7" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder7:', updateError);
                            }
                        }
                    }
                });
