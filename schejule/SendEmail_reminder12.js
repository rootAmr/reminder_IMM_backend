...
export const sendReminder12 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder12 !== 'berhasil12') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder12: 'berhasil12' });
                                console.log('Status updated to "berhasil12" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder12:', updateError);
                            }
                        }
                    }
                });
