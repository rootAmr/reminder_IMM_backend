...
export const sendReminder4 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder4 !== 'berhasil4') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder4: 'berhasil4' });
                                console.log('Status updated to "berhasil4" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder4:', updateError);
                            }
                        }
                    }
                });
