...
export const sendReminder10 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder10 !== 'berhasil10') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder10: 'berhasil10' });
                                console.log('Status updated to "berhasil10" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder10:', updateError);
                            }
                        }
                    }
                });
