...
export const sendReminder9 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder9 !== 'berhasil9') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder9: 'berhasil9' });
                                console.log('Status updated to "berhasil9" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder9:', updateError);
                            }
                        }
                    }
                });
