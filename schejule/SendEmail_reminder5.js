...
export const sendReminder5 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder5 !== 'berhasil5') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder5: 'berhasil5' });
                                console.log('Status updated to "berhasil1" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder5:', updateError);
                            }
                        }
                    }
                });
