...
export const sendReminder3 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_reminder3 !== 'berhasil3') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder3: 'berhasil3' });
                                console.log('Status updated to "berhasil3" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder3:', updateError);
                            }
                        }
                    }
                });
