...
export const sendReminder11 = async () => {
    try {
        ...
        for (const suratizin of suratizinList) {
            ...
            if (suratizin.status_11 !== 'berhasil11') {
                const mailOptions = {
                ...
                transporter.sendMail(mailOptions, async (error, info) => {
                    if (error) {
                        ...
                        if (info.accepted.includes(suratizin.from_email)) {
                            try {
                                await suratizin.update({ status_reminder11: 'berhasil11' });
                                console.log('Status updated to "berhasil11" for:', suratizin.uuid);
                            } catch (updateError) {
                                console.error('Error updating status_reminder11:', updateError);
                            }
                        }
                    }
                });
